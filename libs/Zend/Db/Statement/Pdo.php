...
class Zend_Db_Statement_Pdo extends Zend_Db_Statement implements IteratorAggregate
{
    ...
    public function _execute(array $params = null)
    {
        try {
            ...
            if ($params !== null) {
                $rv = $this->_stmt->execute($params);
            } else {
                $rv = $this->_stmt->execute();
            }
            
            if (getEnv("SQL_LOG")) {
        		$query = $this->_stmt->queryString;
        		$type = "SQL_LOG";
        		$took = sprintf("%.3f", microtime(true) - $start);
        		$msg = join(' ', array($type, $took, $query, "\n"));
        	
				//host info
				$res =  $this->_adapter->getConfig();
				$host = $res['host'];
				$msg = $host . " | " . $msg;
				...
				if($print)
				{
					//if we've setup logging correctly, we should be able to split out the write, read and stats queries into individual log files
					if(getEnv("WRITE_SQL_LOG") && strpos(strtolower($host),'write') !== false)
					{
						error_log($msg, 3, getEnv("WRITE_SQL_LOG"));
					}
					else if(getEnv("LPREAD_SQL_LOG") && strpos(strtolower($host),'lpread') !== false)
					{
						error_log($msg, 3, getEnv("LPREAD_SQL_LOG"));
					}
					else if(getEnv("STATS_SQL_LOG") && strpos(strtolower($host),'stats') !== false)
					{
						error_log($msg, 3, getEnv("STATS_SQL_LOG"));
					}
					else //logging is not setup correclty, but we can still log the old school way
					{
						error_log($msg, 3, getEnv("SQL_LOG"));
					}
				}
        	}
